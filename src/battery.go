...
// handleTestBattery publishes one reading on batteryLastReading so a running
// screen routine can render it, then clears it after DurationSecs.
...
// readBattery returns one reading from the INA219, or an error if the device
// failed to initialize in newHawkeye — the module runs on without a power monitor.
...
// batteryVoltageToPercent linearly maps [BATTERY_MIN_VOLTAGE, BATTERY_MAX_VOLTAGE]
// to [0, 100], clamping outside that range.
